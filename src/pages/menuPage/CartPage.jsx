...
const CartPage = () => {
  ...
  const handleIncrease = async (item) => {
    try {
      const response = await fetch(`https://complete-foodi-client-server-l9jv.onrender.com/carts/${item._id}`, {
        method: "PUT",
        headers: {
    ...
    if (item.quantity > 1) {
      try {
        const response = await fetch(
          `https://complete-foodi-client-server-l9jv.onrender.com/carts/${item._id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
      ...
      alert("Item can't be zero. Try deleting item instead.")
    }
  };
  ...
  const handleDelete =   (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`https://complete-foodi-client-server-l9jv.onrender.com/carts/${item._id}`).then(response => {
          if (response) {
            refetch();
